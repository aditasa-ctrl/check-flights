...
def fetch_response(cfg, debug=False):
    ...
    if "data" not in captured:
        log(
            "ERROR: never captured a SmartSearchCheapFlights response. "
...
def run_once(cfg, debug=False):
    ...
    if not deals:
        log(
            f"No round trips matching '{mode}' under {threshold} {want_currency} "
    ...
    for d in new_deals[:8]:
        ...
        title = (
            f"WIZZ RT {home}<->{d['dest']}  {d['total']:.0f} {cur} "
...
def main():
    ...
    log(
        f"Config: {cfg.get('match_mode')} <= {cfg['max_price']} {cfg['currency']}, "
